<#
.SYNOPSIS
  Builds a source-locked 2026 U.S. Congressional candidate database from FEC data.

.DESCRIPTION
  Primary path: downloads the official FEC 2026 Candidate Master bulk ZIP and parses it locally.
  Fallback path: uses the OpenFEC candidates/search API if ZIP download is blocked or -UseApiOnly is set.
  ...
  No mock data. No hand-entered candidate names. Every output row is sourced from FEC bulk data or OpenFEC API.

.NOTES
  FEC Candidate Master / OpenFEC candidate records are federal filing/listing records.
  They are not the same thing as state-certified ballot-access records.
#>

#requires -Version 5.1
[CmdletBinding()]
param(
    [string]$OutDir = (Join-Path (Get-Location) "election-data-2026"),
    [string]$FecCandidateMasterUrl = "https://www.fec.gov/files/bulk-downloads/2026/cn26.zip",
    [string]$FecApiKey = "",
    [switch]$SkipDownload,
    [switch]$UseApiOnly,
    [switch]$IncludePriorOrNotYet,
...
if ([string]::IsNullOrWhiteSpace($FecApiKey)) {
    if ($env:FEC_API_KEY) { $FecApiKey = $env:FEC_API_KEY } else { $FecApiKey = "DEMO_KEY" }
}

New-Item -ItemType Directory -Path $OutDir -Force | Out-Null
$OutDir = (Resolve-Path $OutDir).Path
$zipPath = Join-Path $OutDir "cn26.zip"
$extractDir = Join-Path $OutDir "fec_candidate_master_2026"
$candidateFile = $null
$useApi = [bool]$UseApiOnly

if (-not (Test-CommandExists -Name "python")) {
    throw "Python 3 is required on PATH. Install Python 3, then rerun this script."
}

if (-not $useApi) {
    try {
        if (-not $SkipDownload) {
            Write-Step "Downloading official FEC Candidate Master: $FecCandidateMasterUrl"
            $headers = @{
                "User-Agent" = "Mozilla/5.0 candidate-db-loader/1.0"
                "Accept"     = "application/zip,application/octet-stream,*/*"
            }
            Invoke-WebRequest -Uri $FecCandidateMasterUrl -OutFile $zipPath -Headers $headers
        } elseif (-not (Test-Path $zipPath)) {
            throw "-SkipDownload was used, but $zipPath does not exist."
        }
        ...
        $candidateFile = Get-ChildItem -Path $extractDir -Recurse -File |
            Where-Object { $_.Extension -match '^\.(txt|csv)$' -and $_.Name -match '^(cn|candidate|cand)' } |
            Sort-Object Length -Descending |
            Select-Object -First 1

        if (-not $candidateFile) {
            $candidateFile = Get-ChildItem -Path $extractDir -Recurse -File |
                Where-Object { $_.Extension -match '^\.(txt|csv)$' } |
                Sort-Object Length -Descending |
                Select-Object -First 1
        }

        if (-not $candidateFile) {
            throw "Could not find a .txt or .csv file inside $zipPath."
        }
    }
    catch {
        Write-Warning "Bulk ZIP path failed: $($_.Exception.Message)"
        Write-Warning "Falling back to OpenFEC API. For production, set a real API key in FEC_API_KEY or pass -FecApiKey."
        $useApi = $true
    }
}

$parserPath = Join-Path $OutDir "parse_fec_candidate_master.py"
$parser = @'
#!/usr/bin/env python3
"""
...
'@
Set-Content -Path $parserPath -Value $parser -Encoding UTF8

$parseArgs = @($parserPath, "--outdir", $OutDir)
if ($useApi) {
    Write-Step "Parsing candidates through OpenFEC API"
    $parseArgs += @("--api-only", "--api-key", $FecApiKey, "--request-delay-ms", [string]$RequestDelayMs)
} else {
    Write-Step "Parsing FEC bulk file: $($candidateFile.FullName)"
    $parseArgs += @("--input", $candidateFile.FullName)
}
if ($IncludePriorOrNotYet) { $parseArgs += "--include-prior-or-notyet" }

& python @parseArgs

Write-Step "Done. Outputs are in: $OutDir"
Write-Host "Primary CSV:  $(Join-Path $OutDir 'congress_candidates_2026_fec_selected.csv')"
Write-Host "Minimal CSV:  $(Join-Path $OutDir 'congress_candidates_2026_minimal.csv')"
Write-Host "SQLite DB:    $(Join-Path $OutDir 'congress_candidates_2026_fec.sqlite')"
