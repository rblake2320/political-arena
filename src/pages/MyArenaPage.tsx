...
function SubscriptionCard({ subscription, onChange }: { subscription: any; onChange: (next: any | null) => void }) {
  ...
  const href = subscription.target_href || "/";
  const location = [subscription.race_state, subscription.race_office, subscription.race_district && `District ${subscription.race_district}`]
    .filter(Boolean)
    .join(" ");
        ...
        <Link to={href} className="group flex min-w-0 flex-1 gap-3">
          ...
          <div className="min-w-0">
            <div className="truncate font-medium text-white group-hover:text-indigo-300">
              {subscription.target_label || subscription.target_id}
            </div>
            <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-zinc-500">
              ...
              {subscription.candidate_party && (
                <>
                  <span className="text-zinc-700">/</span>
                  <span>{subscription.candidate_party}</span>
                </>
              )}
              {subscription.challenge_status && (
                <>
                  <span className="text-zinc-700">/</span>
                  <span>{subscription.challenge_status}</span>
                </>
              )}
            </div>
            {subscription.subscription_type === "challenge" && (subscription.challenger_name || subscription.target_name) && (
              <div className="mt-2 text-xs text-zinc-500">
                {subscription.challenger_name} vs {subscription.target_name}
              </div>
            )}
          </div>
        </Link>
