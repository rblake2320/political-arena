...
describe('red-team report regressions', () => {
  ...
  it('supports password reset with hashed token storage and session invalidation', async () => {
    ...
    const knownToken = `known-reset-${Date.now()}`;
    await env.ARENA_DB.prepare(
      `UPDATE users SET password_reset_token_hash = ?, password_reset_expires_at = ? WHERE id = ?`
    ).bind(await hashToken(knownToken), new Date(Date.now() + 60 * 60 * 1000).toISOString(), user.id).run();

    const reset = await post('/api/auth/reset-password', { token: knownToken, password: NEW_PASSWORD });
